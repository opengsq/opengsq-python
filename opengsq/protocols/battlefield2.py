...
class Battlefield2(ProtocolBase):
    """
    This class represents the Battlefield 2 Protocol. It provides methods to interact with Battlefield 2 game servers.
    Battlefield 2 uses the GameSpy Protocol version 3 for server queries.
    """
    ...
    async def get_status(self) -> Status:
        """
        Asynchronously retrieves the status of the Battlefield 2 game server. The status includes information about the server,
        players, and teams.
...
if __name__ == "__main__":
    ...
    async def main_async():
        bf2 = Battlefield2(host="your.bf2.server.com", port=29900, timeout=5.0)
        server = await bf2.get_status()
        print(server)
